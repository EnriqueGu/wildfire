...
for (quarter in 1:4) {
  ...
  all_model_training[[quarter]] <- lr_model
  ...
  #   
  #   # Create the map
  #   map1 <- tm_shape(mask(lr_prob_fire_raster, UK)) +
  ...
  #   
  #   # Predict the probability of fire occurrences
  #   future_prob_fire <- predict(lr_model, newdata = as.data.frame(future_envCovariates), type = "response")
  #   
  #   # Create a raster from the prediction
  #   future_prob_fire_raster <- rasterFromXYZ(cbind(coordinates(future_envCovariates), future_prob_fire))
  #   
  #   # Set the CRS to match the original data
  #   crs(future_prob_fire_raster) <- crs(future_envCovariates)
  #   
  #   # Mask the probabilities:
  #   future_prob_fire_raster <- mask(future_prob_fire_raster, UK)
  #   
  #   # Create a palette of shades of red
  #   my_palette <- colorRampPalette(c("white", "red"))(10)
  #   
  ...
  #   # Create the map
  #   # Plot the predicted probabilities
  #   if (!is.null(future_prob_fire_raster)) {
  #     map1 <- tm_shape(mask(future_prob_fire_raster, UK)) +
  #       tm_raster(title = "Probability", palette = my_palette, style = 'pretty', n = 10) +
  #       tm_shape(UK) + tm_borders(lwd = 1, col = "black") +
  #       tm_compass(type = "8star", size = 3, position = c("right", "top")) +
  #       tm_scale_bar(position = c("right", "bottom")) +  # Move scale bar
  ...
  #                 legend.position = c("left", "bottom"),
  #                 legend.format = list(fun = function(x) as.character(x)))  # Show data values only in legend
  #     tmap_save(map1, filename = paste("result1/future", type_num, "_lr_Q", quarter, ".png", sep = ""))
  #   }
  # }
}
...
for (quarter in 1:4) {
  # Extract the logistic regression model for the specific quarter
  lr_model <- all_model_training[[quarter]]
  
  # Get the coefficients
  coefs <- coef(lr_model)
  
  # Exclude intercept if it's the first coefficient
  coefs <- coefs[-1]
  
  # Take the absolute value of the coefficients
  absolute_coefs <- abs(coefs)
  
  # Compute the relative importance in percentage
  relative_importance <- absolute_coefs / sum(absolute_coefs) * 100
  
  # Convert to a data frame
  importance_df <- data.frame(
    Quarter = rep(quarter, length(relative_importance)),
